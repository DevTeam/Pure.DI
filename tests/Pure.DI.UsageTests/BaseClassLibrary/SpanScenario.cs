...
$f=Factory overrides may pass stack-only values with `ctx.Override<T>(...)` or `ctx.Let<T>(...)` only inside the current synchronous factory frame. Pure.DI reports `DIE049` if such an override is applied inside a generated delegate or deferred factory.

...
$f=When manual `ctx.Override<T>(...)` or `ctx.Let<T>(...)` calls are used in factory delegates, the usual thread-safety rule still applies: wrap the override and the following injection in `lock (ctx.Lock)` or disable thread safety for known single-threaded compositions. Otherwise Pure.DI reports `DIW013`.
$f=The generated default `Func<ReadOnlySpan<char>, T>` binding does not use shared override state and does not require a manual `lock`; use it when the standard `Func` shape is enough.

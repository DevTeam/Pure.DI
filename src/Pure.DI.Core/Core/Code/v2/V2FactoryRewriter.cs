...
sealed class V2FactoryRewriter(
    IArguments arguments,
    ICompilations compilations,
    DpFactory factory,
    Var var,
    string finishLabel,
    ICollection<V2FactoryRewriter.Injection> injections,
    ICollection<V2FactoryRewriter.Initializer> initializers,
    ...
    private ExpressionStatementSyntax CreateAssignmentExpression(SyntaxNode returnBody, StatementSyntax owner) =>
        triviaTools.PreserveTrivia(owner, SyntaxFactory.ExpressionStatement(
            SyntaxFactory.AssignmentExpression(
                ...
                SyntaxFactory.IdentifierName(var.Name).WithLeadingTrivia(SyntaxFactory.Space).WithTrailingTrivia(SyntaxFactory.Space),
            ...
            && identifierName.Identifier.Text == factory.Source.Context.Identifier.Text)
        {
            switch (node.Name.Identifier.Text)
            {
                case nameof(IContext.Tag):
                    return Visit(SyntaxFactory.ParseExpression($" {var.Injection.Tag.ValueToString()}"));

                case nameof(IContext.ConsumerTypes):
                    List<string> consumers;
                    // ReSharper disable once ConvertIfStatementToConditionalTernaryExpression
                    if (_ctx!.RootContext.Graph.Graph.TryGetOutEdges(var.Node, out var outDeps))
                    {
                        consumers = outDeps

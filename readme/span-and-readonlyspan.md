...
Factory overrides may pass stack-only values with `ctx.Override<T>(...)` or `ctx.Let<T>(...)` only inside the current synchronous factory frame or the current delegate invocation. Delegate arguments such as `T text` can be consumed immediately by method injection, but Pure.DI still reports `DIE049` if an outer stack-only value is captured by the delegate.
